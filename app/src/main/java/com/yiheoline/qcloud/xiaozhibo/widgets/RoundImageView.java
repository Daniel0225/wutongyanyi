...
public class RoundImageView extends ImageView {
    ...
    public RoundImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        type = array.getInt(R.styleable.MyRoundImageView_type, TYPE_CIRCLE);
        scaleType = array.getInt(R.styleable.MyRoundImageView_scale2, SCALE_TYPE_FIT);

        corners_top_left = array.getBoolean(R.styleable.MyRoundImageView_top_left, true);
        corners_top_right = array.getBoolean(R.styleable.MyRoundImageView_top_right, true);
        corners_bottom_left = array.getBoolean(R.styleable.MyRoundImageView_bottom_left, true);
        corners_bottom_right = array.getBoolean(R.styleable.MyRoundImageView_bottom_right, true);

        borderRadius = array.getDimensionPixelSize(R.styleable.MyRoundImageView_borderRadius,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, BODER_RADIUS_DEFAULT,
                        getResources().getDisplayMetrics()));
        aspectRatio = array.getFloat(
                R.styleable.MyRoundImageView_aspectRatio2, 0);
        if (aspectRatio == 0) {
            getImageSize();
        }
        array.recycle();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (type == TYPE_CIRCLE) {
            ...
            int screenWith = MeasureSpec.getSize(widthMeasureSpec);
            int height;
            if (aspectRatio == 0) {
                height = screenWith * imageHeight / imageWidth;
            } else {
                height = (int) (screenWith / aspectRatio);
            }
            setMeasuredDimension(screenWith, height);
        }
    }
